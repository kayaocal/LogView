...
namespace ImGui
{
	...
	IMGUI_API bool			CopyTextButton(const char * str_id, ImVec2 size);
	IMGUI_API bool			MagGlassButton(const char * str_id);
	IMGUI_API bool			MagGlassTextButton(const char * str_id, ImVec2 size);
	IMGUI_API bool			TagTextButton(const char * str_id, ImVec2 size);
	IMGUI_API bool			CopyButton(const char * str_id);
	IMGUI_API bool			TagButton(const char * str_id);
    ...
    IMGUI_API char*         LoadIniSettingsFromDisk();                  // call after CreateContext() and before the first call to NewFrame(). NewFrame() automatically calls LoadIniSettingsFromDisk(io.IniFilename).
...
enum ImGuiTabItemFlags_
{
    ...
    ImGuiTabItemFlags_FileNotFound                  = 1 << 4,   // Append '' to title without affecting the ID, as a convenience to avoid using the ### operator. Also: tab is selected on closure and closure is deferred by one frame to allow code to undo it without flicker.
    ImGuiTabItemFlags_FileUpdated                   = 1 << 5    // Append '' to title without affecting the ID, as a convenience to avoid using the ### operator. Also: tab is selected on closure and closure is deferred by one frame to allow code to undo it without flicker.
};
...
enum ImGuiCol_
{
    ...
    ImGuiCol_TextBG,				// Text Background Color
    ImGuiCol_COUNT
...
enum ImGuiStyleVar_
{
    ...
    ImGuiStyleVar_TextBGWidth,		   // float     TextBGWidthAlign
    ImGuiStyleVar_COUNT
